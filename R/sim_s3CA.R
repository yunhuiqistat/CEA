...
#' @param AR: values of the first off-diaonal element for toeplitz matrices which are set to be the error covariance matrix for Xt and Yt. The error covariances for Xc and Yc are set to be 0.1 times identity matrices.
...
sim_s3CA <- function(nt, nc, p, q, s, AR,
  ...
  for (repe in 1:repetition) {
    ...
    # treatment cross covariance analysis
    ...
    # control cross covariance analysis
    CCA_ctrl <- PMD(cov(Xc, Yc), type="standard", K=3, sumabsu = sqrt(s), sumabsv = sqrt(s), center = FALSE, trace = FALSE)
    ...
    eta_cpca <- PMD(cov(Xt), sumabsv = sqrt(s), sumabsu = sqrt(s), K=2, type = "standard",
                    center = FALSE, trace = FALSE)$d[1] / PMD(cov(Xc), sumabsv = sqrt(s), sumabsu = sqrt(s), K=2, type = "standard",
                                                              center = FALSE, trace = FALSE)$d[1]
    PCA_ctst <- PMD(cov(Xt)-eta_cpca*cov(Xc), sumabsv = sqrt(s), sumabsu = sqrt(s), K=2, type = "standard", center = FALSE, trace = FALSE)
