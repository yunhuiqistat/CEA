...
#'  )+
#'  scale_color_aaas()
sim_cPCA <- function(gnt, gnc, p, q, t,
  ...
  for(repe in 1:repetition){
    ...
    covariates_c <- data.frame(A = rep("A=high", 4*gnc),
                               B = rep(c("B=low","B=high"), each = 2*gnc),
                               C = rep(rep(c("C=low","C=high"), each = gnc), 2))


    if(length(eta) > 1){
      eta <- eta_tuning(Xt = Xt, Xc = Xc, eta_range = eta_range, plot = FALSE, sparse_trt = NULL, sparse_ctst = NULL)$eta_opt
      eta_opt_set <- c(eta_opt_set, eta)
    ...
    if(repe == 1 & save_plot){
      ...
      ggsave(paste("../inst/SavedFigures/", "n",gnt,"_p",p,"_beta",beta_12,"_etaopt.pdf", sep = ""), plot = p_all, width = 8, height = 10, units = "in")
    }
  }
